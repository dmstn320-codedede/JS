...
// 목록 로딩 시 보여줄 로딩 UI(스피너)
const listloader = document.querySelector("#refreshBtn");
...
try{
    ...
    if(!res.ok){
        ...
        // 데이터를 json으로 변환해서 renderPost()함수의 인수로 전달
...
function renderPost(items){
    if(page === 1){
        postEl,innerHTML = ''; //초기 로드거나 새로고침일때 기존 목록 제거
    }
    // 더 이상 로드할 항목이 없는 경우 사용자에게 알림을 표시하고 더보기 버튼을 비 활성화
    if(items.length === 0){
        // ul태그에 소괄호에 있는 내용을 삽입해라
        // beforeend : 선택한 요소 안쪽에서 맨 뒤
        postEl.insertAdjacentHTML('beforeend',`<li class=item><div class = "meta muted">더이상 게시물이 없습니다</div></li> `);
